...
import { getPlatform, getArch, getArchForGO, isProject, cleanVersion } from '../lib'
        ...
        task: async ctx => {
          if (local) {
            ...
            if (!isProject(ctx.location)) {
              throw new Error(
                `${chalk.red(ctx.location)} does not have a ${chalk.red('package.json')}. Did you wish to install IPFS globally?`
              )
            }
          } else {
