...
test.only('UIPFS1: should install ipfs globally', async t => {
  // arrange
  t.plan(1)

  // act
  const { stdout } = await execa('aragon', ['ipfs', 'install'])

  // assert
  t.snapshot(normalizeOutput(stdout))
})

// test uninstalling non-npm installation

// test('should install ipfs globally', async t => {
//   t.plan(1)
